...
void push_stack (stack *this, stack_item item) {
   ...
   /*if (this->capacity == this->size){
      ...
      size_t *data = realloc(this->data, capacity * sizeof *data);
      if(data == NULL) abort();
      memmove(data + capacity - this->size, data + this->capacity
      - this->size, this->size * sizeof *data);
      this->data = data;
      this->capacity = capacity; */
   // this->data[this->size+1]= item;
  // size_t *p = this->data + this->capacity - this->size - 1;
 //  *p = item;
...
stack_item pop_stack (stack *this) {
   DEBUGS ('s', show_stack (this));
  // assert (! empty_stack (this));
   stack_item tmp = this->data[this->size--];
   //this->size--;
...
stack_item peek_stack (stack *this, size_t index) {
   ...
   DEBUGS ('s', show_stack (this));
   //size_t sizer = this->size;
   stack_item BryanisDaBest = this->data[index];
   return BryanisDaBest;
   STUB (return NULL;)
}
